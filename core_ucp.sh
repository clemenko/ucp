...
for ext in $(cat hosts.txt|awk '{print $1}'); do
  until [ $(ssh -o ConnectTimeout=1 core@$ext 'exit' 2>&1 | grep 'timed out' | wc -l) = 0 ]; do echo -n "." ; sleep 5; done
done
#sleep 15
echo ""
...
ssh core@$controller1 "docker run --rm -i --name ucp --privileged -v /var/run/docker.sock:/var/run/docker.sock docker/ucp install --admin-password $password --host-address $controller1"

echo " getting tokens"
MGRTOKEN=$(ssh core@$controller1 "docker swarm join-token -q manager")
WRKTOKEN=$(ssh core@$controller1 "docker swarm join-token -q worker")
...
pdsh -l core -w $node_list "docker swarm join --token $WRKTOKEN $controller1:2377" > /dev/null 2>&1
...
ssh core@$dtr_server 'chmod -R 777 /opt/; systemctl enable rpcbind nfs-server; systemctl start rpcbind nfs-server ; echo "/opt *(rw,sync,no_root_squash,no_all_squash)" > /etc/exports; systemctl restart nfs-server  ' > /dev/null 2>&1
...
pdsh -l core -w $node_list "curl -sk https://$dtr_server/ca -o /etc/pki/ca-trust/source/anchors/$dtr_server.crt; update-ca-trust; systemctl restart docker" > /dev/null 2>&1
